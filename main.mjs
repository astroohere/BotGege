...
async function sambungkanKeWhatsApp() {
  ...
  soket.ev.on("connection.update", async (update) => {
    ...
    if (qr) {
      console.log("[Chisa] QR Code diterima, silakan scan:");
      QRCode.generate(qr, { small: true });
    }
    if (connection === "close") {
      console.log("[Chisa] Koneksi terputus, menyambungkan kembali...");
      console.log(lastDisconnect);
      setTimeout(() => sambungkanKeWhatsApp(), 1000);
    }
    if (connection === "open") {
      console.log("[Chisa] Koneksi tersambung!");
    }
  });
  ...
  for (const pluginPath of pluginFiles) {
    try {
      ...
      if (typeof plugin === "function") {
        await plugin(soket);
        console.log(`[Chisa] plugin dimuat: ${pluginPath}`);
      } else {
        console.warn(
          `[Chisa] Module tidak mengekspor fungsi default: ${pluginPath}`
        );
      }
    } catch (err) {
